    ...
    with engine.begin() as conn:
        for _, row in df_trabajo.iterrows():
            ...
            if fecha_tramite is not None:
                # fecha máxima (ya lo tienes)
                if fecha_operativa_global is None or fecha_tramite > fecha_operativa_global:
                    fecha_operativa_global = fecha_tramite
                # nueva lógica
                if fecha_min is None or fecha_tramite < fecha_min:
                    fecha_min = fecha_tramite
                if fecha_max is None or fecha_tramite > fecha_max:
                    fecha_max = fecha_tramite
